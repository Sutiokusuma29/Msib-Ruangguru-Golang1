...
func Login(id string, name string) string {
	...
	nameStudent := strings.Split(Students, ", ")
		for _, studentList := range nameStudent  {
			parts := strings.Split(studentList, "_")
			if len(parts) == 3 && parts[0] == id && parts[1] == name {
				return fmt.Sprintf("Login berhasil: %s (%s)", parts[1], parts[2])
			}
		}
...
func Register(id string, name string, major string) string {
	...
	getStudent := strings.Split(Students, ", ")
	for _, showStudent := range getStudent {
		parts := strings.Split(showStudent, "_")
...
func GetStudyProgram(code string) string {
	...
	return "Program code not found" //TODO: replace this
}
